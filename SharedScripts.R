...
GenerateLocalResultsAllGOs<-function(
  ...
  out<-c(out,paste0("DimPlot(",SeuratObject,",label = TRUE,group.by='",Identity,"')+FeaturePlot(",SeuratObject,",features = LocalMarker, cols = c('black', 'gold'))\n"))
...
#'
#' @examples
GenerateLocalTreatmentResultsAllGOs<-function(
    ChunkName="Test",
    ResultsDF="merged_seurat.markers[merged_seurat.markers$cluster=='2',]",
    Universe="rownames(merged_seurat)",
    SeuratObject="merged_seurat",
  ...
  out<-c(out,paste0("D<-DotPlot(LocalDiffData,features = base::unique(c(LocalResults$Feature[1:5],LocalResults$Feature[max(length(LocalResults$Feature)-5,1):length(LocalResults$Feature)])))+RotatedAxis()+scale_x_discrete(breaks=base::unique(c(LocalResults$Feature[1:5],LocalResults$Feature[max(length(LocalResults$Feature)-5,1):length(LocalResults$Feature)])),labels=LocalResults[base::unique(c(LocalResults$Feature[1:5],LocalResults$Feature[max(length(LocalResults$Feature)-5,1):length(LocalResults$Feature)])),\"Gene\"])\n"))
  out<-c(out,"print(D)\n")
  out<-c(out,"LocalResults[,'avg_log2FC']<-formatC(LocalResults[,'avg_log2FC'], format = 'e', digits = 2)\n")
  out<-c(out,"LocalResults[,'p_val']<-formatC(LocalResults[,'p_val'], format = 'e', digits = 2)\n")
  out<-c(out,"LocalResults[,'p_val_adj']<-formatC(LocalResults[,'p_val_adj'], format = 'e', digits = 2)\n")
