...
GenerateLocalTreatmentResultsAllGOs<-function(
  ...
  out<-c(out,paste0("DotPlot(LocalDiffData,features = base::unique(c(LocalResults$Feature[1:5],LocalResults$Feature[max(length(LocalResults$Feature)-5,1):length(LocalResults$Feature)])))+RotatedAxis()+scale_x_discrete(breaks=base::unique(c(LocalResults$Feature[1:5],LocalResults$Feature[max(length(LocalResults$Feature)-5,1):length(LocalResults$Feature)])),labels=LocalResults[base::unique(c(LocalResults$Feature[1:5],LocalResults$Feature[max(length(LocalResults$Feature)-5,1):length(LocalResults$Feature)])),\"Gene\"])\n"))
  out<-c(out,"LocalResults[,'avg_log2FC']<-formatC(LocalResults[,'avg_log2FC'], format = 'e', digits = 2)\n")
  out<-c(out,"LocalResults[,'p_val']<-formatC(LocalResults[,'p_val'], format = 'e', digits = 2)\n")
  out<-c(out,"LocalResults[,'p_val_adj']<-formatC(LocalResults[,'p_val_adj'], format = 'e', digits = 2)\n")
  out<-c(out,"DT::datatable(LocalResults[,c('gene','pct.1','pct.2','avg_log2FC','p_val','p_val_adj')],colnames = c('Gene','pct in cluster','pct out of cluster','avg log2FC','p val','p val adj'),rownames = FALSE, extensions = 'Buttons', options = list( dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel')),escape=FALSE)\n")
